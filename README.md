...
- **禅意阅读**：纸墨质感排版（宣纸白 / 墨黑 / 竹青 / 檀棕 / 暮云灰，禁用纯黑白）、宣纸颗粒纹理三档（WebGL，可关）、字号/主题（浅·暖·暗）/字体切换、段首缩进与两端对齐（还原纸质书密排）、目录（滚动追踪高亮当前章节）、工具栏随滚动智能隐现、代码块语法高亮与一键复制、mermaid 图表渲染（懒加载，随主题重绘）、LaTeX 公式渲染（KaTeX，行内 `$…$` 与行间 `$$…$$`、`\begin{}` 环境，字色随纸墨主题）、链接分流（外链开系统浏览器，文档互链应用内跳转）、键盘阅读流（`j`/`k` 滚动、`空格` 翻页、`←`/`→` 章节跳转、`t` 目录、`n` 笔记、`z` 禅境，「键径」速查随手可唤）、禅境模式（一键隐去全部 UI，可选「沉浸全屏」入定自动全屏、出定一并退去，Esc 层层退返）。
- **入定动画（六档任选）**：进入禅境不是过场动画，而是一场可换风格的入定仪式——默认「墨韵」：纸纱先起，一滴墨坠入屏心，笔锋逆时针书就一枚圆相（飞墨随锋绽开、末段飞白），三息呼吸与之同频，**每次呼气世界真实地退去一层**（顶栏化去 → 面板隐去边距舒展 → 氛围光起正文澄明），末息圆相收作墨点沉入屏心；另有「星河入砚」（星河旋作一泓，万点归一凝作星滴坠入砚心）、「落叶听禅」（一叶知秋、风起万叶，暮色四合，万叶栖水）与「香篆引定」（万点香尘落就一篆，火星循篆燃行、青烟袅起，篆尽烟敛归一）几种意象，「轻雾速入」则是约 1 秒的快速短雾，「随机」每次入定在仪式档中轮换。调适面板可自由切换、「试播」先睹；一声入定钵音轻响，轻触任意处随时跳过，Esc 随时出定。
- **觉悟笔记**：选中文本「驻足」或「写下觉悟」，用你自己的话重述；心得可书 Markdown（与正文同源渲染：代码高亮、公式、mermaid、表格、图片灯箱），弹窗内「写 / 观」即时预览；笔记持久化，刷新后高亮与笔记均保留。
...
npm run test:unit  # Vitest 单元测试（jsdom，覆盖 markdown 渲染管线）
```
